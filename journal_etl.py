...
class ProcessJournal:
    ...
    def extract_works(self):
        ...
        self.fm.frame_maker(entity={entity: None},
                            filtre={'locations.source.issn': self.journal_issn},
    ...
    def extract_cited(self):
        ...
        entity = 'works'
        master_dict = defaultdict(list)
        for j, (works_id, referenced_id) in enumerate(zip(cited.works_id, cited.referenced_works), start=1):
            ...
            for name, df in self.fm.frame_dict.items():
                try:
                    df.insert(0, 'citing_work_id', works_id)
                    master_dict[name].append(df)
                except Exception as e:
                    print(f'could not insert citing_work_id column {e = }')
