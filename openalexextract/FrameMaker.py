...
class FrameMaker(object):
    ...
    def set_parents(self):
        self.parent_template = {'works': {'authorships': self.frame_from_list_of_dict,
                                          'primary_location': self.frame_from_list_of_dict,
    ...
    def frame_maker(self, cache_name=None, entity=None, filtre=None, select=None, refresh=None):

        self.query = self.qm.query_maker(entity=entity, filtre=filtre)
        ...
        self.response = self.ex.extractor(cache_name=cache_name, query=self.query, refresh=refresh)
    ...
    def get_parents(self):
        ...
        self.df = pd.json_normalize(self.response, max_level=99)
        ...
        for col, select in parents.items():
            # print(f'parents: {col = }')
            if any(col.startswith(c) for c in self.df.columns):
                # print(f'selected: {col = } {select = }')
                select(col=col)
    ...
    def frame_from_list_of_dict(self, col=None):
        ...
        if col_ext:
            if '.' not in col_ext:
                try:
                    temp = temp.explode(col_ext).reset_index()
                    ...
                    print(f'explode of col_ext FAILED {col_ext = } with exception {e = }')
            else:
        ...
        # if 'authorships' == col:
        #     print(temp[['works_id', 'author_id', 'author_display_name', 'raw_affiliation_string',
        #                 'institutions_id', 'institutions_display_name', 'country_code']].head(8))
